...
def vnd_intra(sol, show_operations=False):
    ...
    for k in range(sol['instance']['l']):
        imp_route = True
        while imp_route:
            imp_route = intra_route.shift(sol, k, 'first')
            if imp_route:
                if show_operations:
                    print("Intra: Shift")
                improvement = True
            if not imp_route:
                imp_route = intra_route.swap(sol, k, 'first')
                if imp_route:
                    if show_operations:
                        print("Intra: Swap")
                    improvement = True
            if not imp_route:
                imp_route = intra_route.two_opt(sol, k, 'first')
                if imp_route:
                    if show_operations:
                        print("Intra: 2-opt")
                    improvement = True
...
def vnd_inter(sol, show_operations=False):
    ...
    imp = True
    while imp:
        imp = inter_route.shift(sol, 'first')
        if imp:
            if show_operations:
                print("Inter: Shift")
            improvement = True
        if not imp:
            imp = inter_route.swap(sol, 'first')
            if imp:
                if show_operations:
                    print("Inter: Swap")
                improvement = True
